...
namespace BrnMall.Services
{
    public partial class MallUtils
    {
        ...
        public static void SetUserCookie(PartUserInfo partUserInfo, int expires)
        {
            ...
            cookie.Values["openid"] = partUserInfo.Openid.ToString();
            cookie.Values["pid"] = partUserInfo.Pid.ToString();
            cookie.Values["userno"] = partUserInfo.Userno.ToString();
            cookie.Values["userlevel"] = partUserInfo.UserLevel.ToString();
            if (expires > 0)
            {
