...
{
    public class WeiXinHelp : System.Web.UI.Page
    {
        ...
        public static string IsExistjsapi_ticket()
        {
            ...
            string Ticket = "";
            object JSAPI_Ticket = Common.CacheHelper.GetCache("JSAPI_Ticket");//设置主菜单缓存  防止多次访问数据库V1003_TODAY_MUSIC
            ...
            {
                Ticket = Getjsapi_ticket();//重新获取Access_Token
               Common.CacheHelper.SetCache("JSAPI_Ticket", Ticket, TimeSpan.FromMinutes(120));
            }
            return Ticket;
        }
        ...
        {
            string AccessTocken = IsExistAccess_Token2();//获取最新有效的AccessToken 
            string CommUrl = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token=" + AccessTocken + "&type=jsapi";
            string content = GetPage(CommUrl, "");
            string ticket = GetJsonValue(content, "ticket");
            string errmsg = GetJsonValue(content, "errmsg");
            string errcode = GetJsonValue(content, "errcode");
