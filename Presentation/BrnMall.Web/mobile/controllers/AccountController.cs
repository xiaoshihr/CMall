...
namespace BrnMall.Web.Mobile.Controllers
{
    ...
    public partial class AccountController : BaseMobileController
    {
        ...
        public ActionResult Login()
        {
            ...
            else if (accountName.Length <1 || accountName.Length > 50)
            {
                errorList.AppendFormat("{0}\"key\":\"{1}\",\"msg\":\"{2}\"{3},", "{", "accountName", "账户名必须大于1且不大于50个字符", "}");
            }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult RegStore()
        {
            if (WorkContext.UserLevel < 3)
                return PromptView("抱歉,只有钻石会员才能入驻开店");
            if (WorkContext.StoreId > 0)
                return PromptView("您已入驻");
        }
