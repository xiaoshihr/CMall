...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetProductList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[storeid],[temp1].[name] AS [pname],[temp1].[shopprice],[temp1].[showimg],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp2].[number],[temp3].[name] AS [sname] FROM (SELECT TOP {0} [pid],[storeid],[name],[shopprice],[showimg],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder] FROM [{1}products] ORDER BY [displayorder] DESC,[pid] DESC) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid] LEFT JOIN [{1}stores] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);
