...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        /// <param name="userName">用户名</param>
        /// <returns></returns>
        public IDataReader GetPartUserByOpenid(string Openid)
        {
            DbParameter[] parms = {
                                       GenerateInParam("@Openid",SqlDbType.VarChar,100, Openid)
                                   };
            return RDBSHelper.ExecuteReader(CommandType.StoredProcedure,
                                            string.Format("{0}getpartuserbyOpenid", RDBSHelper.RDBSTablePre),
                                            parms);
        }
        ...
        public int CreateUser(UserInfo userInfo)
        {
            DbParameter[] parms = {
									   ...
									   GenerateInParam("@avatar",SqlDbType.VarChar,300,userInfo.Avatar),
                                       ...
                                       GenerateInParam("@Openid",SqlDbType.VarChar,100,userInfo.Openid),
                                       GenerateInParam("@Pid",SqlDbType.Int,4,userInfo.Pid),
                                       //GenerateInParam("@Userno",SqlDbType.Int,4,userInfo.Userno),
                                       GenerateInParam("@UserLevel",SqlDbType.Int,4,userInfo.UserLevel),
        ...
        public void UpdatePartUser(PartUserInfo partUserInfo)
        {
            DbParameter[] parms = {
                                       ...
                                       GenerateInParam("@salt",SqlDbType.NChar,6,partUserInfo.Salt),
                                        GenerateInParam("@Openid",SqlDbType.VarChar,100,partUserInfo.Openid),
                                       GenerateInParam("@Pid",SqlDbType.Int,4,partUserInfo.Pid),
                                       //GenerateInParam("@Userno",SqlDbType.Int,4,partUserInfo.Userno),
                                       GenerateInParam("@UserLevel",SqlDbType.Int,4,partUserInfo.UserLevel),
        ...
        public DataTable AdminGetUserList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}users].[Openid],[{1}users].[Pid],[{1}users].[Userno],[{1}users].[UserLevel],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users] LEFT JOIN [{1}userdetails] ON [{1}userdetails].[uid] = [{1}users].[uid]  LEFT JOIN [{1}userranks] ON [{1}userranks].[userrid]=[{1}users].[userrid]  LEFT JOIN [{1}malladmingroups] ON [{1}malladmingroups].[mallagid]=[{1}users].[mallagid] ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}users].[Openid],[{1}users].[Pid],[{1}users].[Userno],[{1}users].[UserLevel],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users] LEFT JOIN [{1}userdetails] ON [{1}userdetails].[uid] = [{1}users].[uid]  LEFT JOIN [{1}userranks] ON [{1}userranks].[userrid]=[{1}users].[userrid]  LEFT JOIN [{1}malladmingroups] ON [{1}malladmingroups].[mallagid]=[{1}users].[mallagid] WHERE {2} ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[storeid],[{1}users].[mallagid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}users].[Openid],[{1}users].[Pid],[{1}users].[Userno],[{1}users].[UserLevel],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}malladmingroups].[title] AS [atitle] FROM [{1}users],[{1}userdetails],[{1}userranks],[{1}malladmingroups]  WHERE [{1}userdetails].[uid] = [{1}users].[uid] AND  [{1}userranks].[userrid]=[{1}users].[userrid] AND  [{1}malladmingroups].[mallagid]=[{1}users].[mallagid] AND [{1}users].[uid] < (SELECT min([uid])  FROM (SELECT TOP {2} [uid] FROM [{1}users] ORDER BY [uid] DESC) AS temp ) ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize);
                else
        ...
        public string AdminGetUserListCondition(string userName, string email, string mobile, int userRid, int mallAGid)
        {
            ...
            if (!string.IsNullOrWhiteSpace(userName) && SecureHelper.IsSafeSqlString(userName))
                condition.AppendFormat(" AND ( [{1}users].[username] like '{0}% ' or [{1}users].[userno] like '{0}% '  ) ", userName, RDBSHelper.RDBSTablePre);
            ...
            if (userRid > 0)
                condition.AppendFormat(" AND [{1}users].[userlevel] = {0} ", userRid, RDBSHelper.RDBSTablePre);
